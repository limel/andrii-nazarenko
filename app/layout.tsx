import type { Metadata } from "next";
import { IBM_Plex_Sans } from "next/font/google";
import "./globals.css";

const ibmPlexSans = IBM_Plex_Sans({
  variable: "--font-ibm-plex-sans",
  subsets: ["latin"],
});

export const metadata: Metadata = {
  title: "Andrii Nazarenko - Frontend Engineer",
  description:
    "Resume of Andrii Nazarenko, a frontend engineer specializing in building scalable systems for modern product applications.",
};

export default function RootLayout({
  children,
}: Readonly<{
  children: React.ReactNode;
}>) {
  return (
    <html
      lang="en"
      className={`${ibmPlexSans.variable} h-full bg-stone-50 antialiased dark:bg-zinc-900`}
    >
      <body className="flex min-h-full flex-col font-sans">
        <div aria-hidden="true" className="pointer-events-none fixed inset-0 overflow-hidden">
          <div className="anim-blob-1 bg-accent absolute -top-72 -left-72 h-200 w-200 rounded-full opacity-[0.06] blur-[160px] dark:opacity-[0.10]" />
          <div className="anim-blob-2 absolute -right-72 -bottom-72 h-175 w-175 rounded-full bg-[#6d28d9] opacity-[0.04] blur-[140px] dark:opacity-[0.08]" />
        </div>
        {children}
      </body>
    </html>
  );
}
